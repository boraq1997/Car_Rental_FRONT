  ...
  <div class="card" dir="rtl">
    ...
    >
      ...
      <Column field="status" header="حالة العجلة" sortable>
        ...
        <template #body="{ data }">
          <Tag v-if="data.status === 'available'" icon="fas fa-car-side" severity="info" value="متاح" />
          <Tag v-if="data.status === 'rented'" icon="fas fa-hourglass-start" severity="warn" value="مستاجر" />
          <Tag v-if="data.status === 'maintenance'" icon="fas fa-car-burst" severity="error" value="صيانة" />
        </template>
      </Column>
      ...
      <Column header="تفاصيل اكثر">
        ...
        <template #body="{ data }">
          <Button icon="fa-solid fa-arrow-up-right-from-square" severity="success" rounded variant="outlined" class="p-button-sm" />
          <Button
    ...
    <Dialog
      ...
      :header="isEditMode ? 'حفظ التعديلات' : 'إضافة عجلة جديدة'"
      :style="{ width: '30vw' }"
      modal
      @hide="resetForm"
      dir="rtl"
    >
      ...
      <!-- ====== Footer Buttons ====== -->
      <template #footer>
        <Button label="الغاء" icon="fas fa-times" severity="secondary" class="p-button-sm" @click="addEditVehiclesFormDialogVisible = false" />
        <Button :label="isEditMode ? 'حفظ' : 'إضافة'" icon="fas fa-floppy-disk" :loading="isSaving" class="p-button-sm" @click="saveNewVehicle()" />
      </template>
    </Dialog>
  </div>
</template>
  ...
  confirm.require({
    ...
    accept: async () => {
      try {
        ...
        toast.add({ severity: "success", summary: "Deleted", detail: "Vehicle deleted successfully", life: 3000 });
        fetchAllVehicles();
      } catch (err) {
